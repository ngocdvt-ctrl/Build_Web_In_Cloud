...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (!apiKey || !mailFrom) {
    // Không leak thêm gì, nhưng phải báo lỗi cấu hình
  ...
  try {
    ...
    // Ẩn tồn tại/không tồn tại để tránh leak
    ...
    // Check expiry (nếu hết hạn thì không gửi)
    ...
    if (elapsedSec < COOLDOWN_SEC) {
      ...
      await client.query("COMMIT");
      // Trả 200 để UX tốt (không coi là lỗi), và tránh user spam retry
    ...
    // Gửi mail (vẫn trong transaction; row đang lock -> chống race)
    await sgMail.send({
      to: normalizedEmail,
    ...
    // Update last sent timestamp (throttle)
    await client.query(
      `
      UPDATE users
